...
| DOC-03 | Document | Close document | MVP | NOT_STARTED | | Document | | | | Not domain logic — dropping a `PdfDocument` value; nothing to test in isolation | ADR-001 unresolved |
...
| VIEW-01 | Viewing | Zoom | MVP | IN_PROGRESS | | | Rendering | `cargo test -p pdf_core` (render_tile_to_png tests render at arbitrary zoomed_width) | `crates/pdf_core/src/lib.rs` | Rendering-at-a-zoom-level works (tiling); the interactive zoom UI/gesture itself is not built | ADR-001 unresolved; UI needs Tauri |
| VIEW-02 | Viewing | Pan | MVP | NOT_STARTED | | | Rendering | | | Tile addressing (tile_x/tile_y) supports pan in principle; no pan interaction built | ADR-001 unresolved; UI needs Tauri |
...
| RFI-01 | RFI/Revision | Create RFI tied to page/markup | MVP | NOT_STARTED | | RFI | | | | Elevated from backlog #9 per Naveen's stated priority | Collaboration model not confirmed |
| RFI-02 | RFI/Revision | RFI status tracking | MVP | NOT_STARTED | | RFI | | | | | Collaboration model not confirmed |
...
| EXPORT-01 | Export | Flattened PDF export | MVP | NOT_STARTED | | | Export | | | | ADR-001 unresolved |
